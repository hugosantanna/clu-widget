#!/usr/bin/env python3
"""
claude_usage.py — tiny Claude usage widget for your terminal split-pane.
Reads your Claude Code OAuth token (no extra setup needed) and shows
live 5-hour + 7-day usage against Anthropic's servers.

Requirements: pip install rich requests keyring --break-system-packages
Usage:        python3 claude_usage.py
              python3 claude_usage.py --refresh 60   # custom interval (seconds)
"""
...
try:
    ...
    print("Missing: pip install rich requests keyring --break-system-packages")
    sys.exit(1)
...
# ── The creature ─────────────────────────────────────────────────────────────
# Frames for the idle creature (displayed most of the time)
CREATURE_IDLE = [
    # antenna, head, face, legs
    [
        f"          [{VIOLET}]*[/]",
        f"          [{VIOLET}]|[/]",
        f"        [{SKIN}]┌────┐[/]",
...
# Frames for the bounce (played over ~2 seconds)
CREATURE_BOUNCE = [
    # frame 1: squat
    [
        f"",
...
BOUNCE_INTERVAL = 120  # ticks between bounces (at 2fps = 60 seconds)
BOUNCE_FRAME_HOLD = 3  # how many ticks to hold each bounce frame
    ...
    if sys.platform == "darwin":
        ...
        for svc in services:
            try:
                ...
                # Handle JSON-wrapped tokens (e.g. Claude Code-credentials)
                if out.startswith("{"):
                    try:
                        blob = json.loads(out)
...
def _setup_terminal():
    ...
    # Register cleanup so cursor is always restored
    atexit.register(_cleanup)

    # Set window title
    sys.stdout.write(f"\033]0;claude·usage\007")

    # Resize terminal window (rows ; cols)
...
def main():
    ...
    if not token:
        console.print(Panel(
            Text.from_markup(
                ...
                f"  [bold {CYAN}]CLAUDE_TOKEN=sk-ant-… python3 claude_usage.py[/]\n"
            ),
            border_style=DIM,
            box=box.SIMPLE,
        ))
    ...
    # Set up widget mode: clear, resize, hide cursor
    if not args.no_resize:
        _setup_terminal()
    else:
        # Still clear and hide cursor, just don't resize
        atexit.register(_cleanup)
        sys.stdout.write("\033[2J\033[H\033[?25l")
        sys.stdout.write(f"\033]0;claude·usage\007")
        sys.stdout.flush()

    next_fetch = 0  # fetch immediately on first tick

    with Live(make_widget(data, last_ok, error_msg, tick),
              console=console,
        ...
        while True:
            ...
            if now_ts >= next_fetch:
                try:
                    ...
                    error_msg = str(e)[:36]
                    next_fetch = now_ts + 10  # retry sooner on error

            live.update(make_widget(data, last_ok, error_msg, tick))
            tick += 1
            time.sleep(0.5)
